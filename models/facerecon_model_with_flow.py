...
from util.preprocess_with_flow import estimate_norm_torch
...
class FaceReconModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.input_img_pre = input["imgs_pre"].to(self.device) if "imgs_pre" in input else None
        self.gt_lm_pre = input["lms_pre"].to(self.device) if "lms_pre" in input else None

        self.s = input["s"] if "s" in input else None
        self.s_pre = input["s_pre"] if "s_pre" in input else None
        self.list_vertex = input["list_vertex"] if "list_vertex" in input else None
        self.list_vertex_pre = input["list_vertex_pre"] if "list_vertex_pre" in input else None
        self.W = input["W"] if "W" in input else None
        self.H = input["H"] if "H" in input else None

    def forward(self):
        output_coeff = self.net_recon(self.input_img)
        self.facemodel.to(self.device)
        _, self.pred_vertex, self.pred_tex, self.pred_color, self.pred_lm = self.facemodel.compute_for_render(
            output_coeff
        )
        self.pred_mask, _, self.pred_face, _ = self.renderer(
            self.pred_vertex, self.facemodel.face_buf, feat=self.pred_color
        )

        self.pred_coeffs_dict = self.facemodel.split_coeff(output_coeff)
        return self.pred_vertex

    def compute_flow_coeff(self, coeff, coeff_pre):
        self.facemodel.to(self.device)
        self.id, self.face_vertex, _, _, _ = self.facemodel.compute_for_render(coeff)
        self.pre_id, self.pre_face_vertex, _, _, _ = self.facemodel.compute_for_render(coeff_pre)

        self.id = self.id.squeeze()
        self.pre_id = self.pre_id.squeeze()

        H, W = self.id.shape[0], self.id.shape[1]
        self.flow_u = np.zeros((H, W))
        self.flow_v = np.zeros((H, W))
        for i in range(int(self.id.min()), int(self.id.max()) + 1):
            h_n, w_n = int(torch.where(self.id == i)[0]), int(torch.where(self.id == i)[1])
            h_p, w_p = int(torch.where(self.pre_id == i)[0]), int(torch.where(self.pre_id == i)[1])
            self.flow_u[h_p][w_p] = h_n - h_p
            self.flow_v[h_p][w_p] = w_n - w_p
        return self.flow_u, self.flow_v

        # self.pred_coeffs_dict = self.facemodel.split_coeff(coeff)

    def compute_flow(self):
        coeff = self.net_recon(self.input_img)
        coeff_pre = self.net_recon(self.input_img_pre)
        self.facemodel.to(self.device)
        self.id, self.face_vertex, _, _, _ = self.facemodel.compute_for_render(coeff)
        self.pre_id, self.pre_face_vertex, _, _, _ = self.facemodel.compute_for_render(coeff_pre)
        # self.id, self.pre_id = self.id.squeeze(), self.pre_id.squeeze()
        self.id[..., 1] = 224 - self.id[..., 1]
        self.pre_id[..., 1] = 224 - self.pre_id[..., 1]
        self.id = self.id.squeeze()
        self.pre_id = self.pre_id.squeeze()
        x, y = self.list_vertex[0], self.list_vertex[2]
        # offset = torch.zeros(self.id.shape[1], 2).to(self.device)
        # for i in range(self.id.shape[0]):
        x_n = torch.full((self.id.shape[0], 1), int(x))
        y_n = torch.full((self.id.shape[0], 1), int(y))
        offset = torch.cat((x_n, y_n), axis=1).to(self.device)
        # x_i = torch.full((self.input_img.shape[0], self.H, self.W, 1), x)
        # y_i = torch.full((self.input_img.shape[0], self.H, self.W, 1), y)
        # offset_i = torch.cat((x_i, y_i), axis=3).to(self.device)
        # self.id = self.id + offset
        # self.id = self.id*self.s
        x_pre, y_pre = self.list_vertex_pre[0], self.list_vertex_pre[2]
        # offset_pre = torch.zeros(self.pre_id.shape[0], self.pre_id.shape[1], 2).to(self.device)
        # for i in range(self.pre_id.shape[0]):
        x_n_pre = torch.full((self.pre_id.shape[0], 1), int(x_pre))
        y_n_pre = torch.full((self.pre_id.shape[0], 1), int(y_pre))
        offset_pre = torch.cat((x_n_pre, y_n_pre), axis=1).to(self.device)
        self.pre_id = self.pre_id + offset_pre
        self.pre_id = self.pre_id / self.s_pre.to(torch.float32).to(self.device)
        # self.pre_id = self.pre_id*self.s.to(torch.float32).to(self.device)/self.s_pre.to(torch.float32).to(self.device) - offset

        self.id = self.id + offset
        self.id = self.id / self.s.to(torch.float32).to(self.device)

        self.flow = self.pre_id - self.id

        self.pred_mask, _, self.pred_face, _ = self.renderer(self.face_vertex, self.facemodel.face_buf, feat=self.flow)
        # for i in range(self.id.shape[0]):
        self.pred_face = torch.nn.functional.pad(
            self.pred_face,
            (
                int(self.list_vertex[0]),
                int(self.W * self.s) - int(self.list_vertex[1]),
                int(self.list_vertex[2]),
                int(self.H * self.s) - int(self.list_vertex[3]),
            ),
        )
        self.pred_mask = torch.nn.functional.pad(
            self.pred_mask,
            (
                int(self.list_vertex[0]),
                int(self.W * self.s) - int(self.list_vertex[1]),
        ...
        # B, H, W = self.input_img.shape[0], self.input_img.shape[2], self.input_img.shape[3]

        # self.flow_map = torch.zeros((B, H, W, 2))

        # for k in range(B):
        #     for i in range(self.id.shape[1]):
        #         self.flow_map[k][min(int(self.pre_id[k][i][0])-1, 223)][min(int(self.pre_id[k][i][1])-1, 223)] = self.flow[k][i]
